  ...
  <view class="timeline">
    ...
    >
      ...
      >
        ...
        <!-- 轴线内容（图标、标题、时间） -->
        <view class="timeline-item__axis-content" v-if="displayMode === 'tree'">
        ...
        <view v-if="displayMode === 'tree' && step">
          ...
          >
            ...
            >
              ...
              <view
                v-if="
                  imgIdx === (currentImageIndexes[index] || 0) && getStepImages(index).length > 1
                "
...
const getStepStatus = index => {
  const step = properties.processData[index]
  if (step && step.status) {
    return step.status
  }
  // 如果没有status，则使用currentStep逻辑
  if (index < properties.currentStep) {
    return 'completed'
  } else if (index === properties.currentStep) {
    return 'in-progress'
  } else {
    return 'pending'
  }
...
const getGlobalImageIndex = nodeIndex => {
  let index = 0
  for (let index_ = 0; index_ < properties.processData.length; index_++) {
    const images = getStepImages(index_)
    if (index_ < nodeIndex) {
      index += images.length
    } else if (index_ === nodeIndex) {
      index += currentImageIndexes.value[index_] || 0
      break
    }
  }
  return index
...
watch(
  ...
  value => {
    currentImageIndexes.value = value.map(() => 0)
  },
  { immediate: true }
)
...
const getStackedImageStyle = (imgIndex, activeIndex, direction = 'left') => {
  const offset = Math.min(imgIndex, 3)
  const isActive = imgIndex === activeIndex
  // 左侧往左上，右侧往右下
...
const getVerticalStackedImageStyle = (imgIndex, activeIndex) => {
  const offset = Math.min(imgIndex, 3)
  const isActive = imgIndex === activeIndex
  return {
    position: 'absolute',
