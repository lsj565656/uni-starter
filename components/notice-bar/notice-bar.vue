<template>
  <view class="custom-notice-bar" :style="{ backgroundColor: backgroundColor }">
    <view class="notice-content">
      ...
      <text class="notice-time" :class="{ 'fade-out': isTransitioning }">
...
const currentNotice = computed(() => {
  return properties.notices[currentNoticeIndex.value] || null
})
...
const switchToNextNotice = () => {
  ...
  setTimeout(() => {
    // 切换到下一条消息
    currentNoticeIndex.value = (currentNoticeIndex.value + 1) % properties.notices.length
...
const startAutoSwitch = () => {
  if (properties.notices.length <= 1) return
