    ...
    <view class="comment-main">
      ...
      <view class="comment-actions">
        ...
        <view class="like-btn" @click="$emit('like', comment.id)">
          <uni-icons
            :type="comment.is_liked ? 'heart-filled' : 'heart'"
            size="16"
            :color="comment.is_liked ? 'red' : '#bbb'"
          />
      ...
      <view v-if="comment.replies && comment.replies.length > 0" class="replies">
        ...
        <template v-else>
          ...
          <!-- 加载更多回复 -->
          <view
            v-if="comment.hasMoreReplies && !comment.loadingReplies"
          ...
          <!-- 收起回复 -->
          <view
            v-if="comment.reply_count > 2 && comment.replies.length >= comment.reply_count"
            class="collapse-replies"
            @click="$emit('collapse-replies', comment.id)"
          >
