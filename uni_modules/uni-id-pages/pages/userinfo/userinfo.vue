...
<template>
  <view class="uni-content">
    ...
    <uni-list>
      ...
      <uni-list-item class="item" @click="setGender" title="性别" :rightText="getGenderText(userInfo.gender) || '未设置'"
        link>
      </uni-list-item>
    ...
    <!-- #ifndef MP -->
    <uni-list class="mt10">
      <uni-list-item @click="handleLogout" :title="$t('settings.logOut')" link="navigateTo"></uni-list-item>
    </uni-list>
    <uni-list class="mt10">
    ...
    <!-- 性别选择弹窗 -->
    <uni-popup ref="genderDialog" type="dialog">
      <uni-popup-dialog mode="base" title="选择性别" @confirm="confirmGender">
        <view class="gender-options">
          <view v-for="option in genderOptions" :key="option.value" class="gender-option"
            :class="{ active: selectedGender === option.value }" @click="selectedGender = option.value">
            <text class="gender-text">{{ option.text }}</text>
          </view>
        </view>
      </uni-popup-dialog>
    </uni-popup>
...
export default {
  ...
  methods: {
    ...
    // 设置性别
    setGender() {
      this.selectedGender = this.userInfo.gender || 0
