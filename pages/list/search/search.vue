<template>
  <view class="container">
    ...
    <view class="search-body">
      <!-- 搜索历史 -->
      <view class="word-container" v-if="localSearchList.length > 0">
      ...
      <view class="word-container">
        ...
        <unicloud-db
          ...
          :page-size="10"
        >
          <text v-if="loading && !netHotListIsHide" class="word-container_body-info"
            >正在加载...</text
          >
 ...
 */
const searchLogDatabaseName = 'opendb-search-log' // 搜索记录数据库
const mallGoodsDatabaseName = 'opendb-news-articles' // 文章数据库
...
const arrayUnique = array => {
  for (let index = array.length - 1; index >= 0; index--) {
    const currentIndex = array.indexOf(array[index])
    const lastIndex = array.lastIndexOf(array[index])
    currentIndex != lastIndex && array.splice(lastIndex, 1)
  }
  return array
} // 节流
// 防抖
function debounce(function_, interval, isFirstAutoRun) {
  /**
   *
   * @param {要执行的函数} fn
   * @param {在操作多长时间后可再执行，第一次立即执行} interval
   */
  const _self = function_
  let timer = null
  let first = true
...
export default {
  data() {
    return {
      mallGoodsDbName: mallGoodsDatabaseName,
      searchLogDbName: searchLogDatabaseName,
      statusBarHeight: '0px',
  ...
  methods: {
    ...
    localSearchListManage(word) {
      const list = uni.getStorageSync(localSearchListKey)
      if (list.length > 0) {
        this.localSearchList.unshift(word)
        arrayUnique(this.localSearchList)
    ...
    LocalSearchlistItemClick(word, index) {
      if (this.localSearchListDel) {
        ...
        uni.setStorageSync(localSearchListKey, this.localSearchList)
        if (this.localSearchList.length === 0) {
          this.localSearchListDel = false
        }
        return
      }
      ...
      return new Promise((resolve, reject) => {
        ...
        if (uniId) {
          ...
          plus.device.getInfo({
            ...
            fail: () => {
              resolve(uni.getSystemInfoSync().system + '_' + Math.random().toString(36).slice(2))
            }
          })
          // #endif
          // #ifndef APP-PLUS
