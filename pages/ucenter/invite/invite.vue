    ...
    <view class="box">
      ...
      <view @click="download" id="download">
        <image
          v-if="isIos"
          class="icon"
          src="@/static/h5/download-app/ios.png"
          mode="widthFix"
        ></image>
        <image
          v-else
          class="icon"
          src="@/static/h5/download-app/android.png"
          mode="widthFix"
        ></image>
...
export default {
  ...
  created() {
    ...
    this.isWeixin = ua.includes('micromessenger')
    //判断是否在ios或者安卓打开
    this.isIos = !!/\(i[^;]+;( U;)? CPU.+Mac OS X/.test(userAgent)
  },
  onLoad({ code }) {
    this.code = code
      ...
      if (this.code) {
        uni.setClipboardData({
          ...
          complete: e => {
            ...
            document.querySelector('##clipboard').style.top = '-999px'
            uni.hideKeyboard()
          }
        })
      }
