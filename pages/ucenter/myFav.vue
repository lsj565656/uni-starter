...
async function fetchTasks({ reset = false } = {}) {
  ...
  try {
    ...
    if (res.result && res.result.code === 0) {
      ...
      const flatList = rawList
        .map(item =>
          item.task ? { ...item.task, is_liked: true, like_count: item.task.like_count } : null
        )
        .filter(Boolean)
      tasksList.value = reset ? flatList : [...tasksList.value, ...flatList]
...
function syncLikeMapWithData(data) {
  ...
  const databaseIds = new Set((data || []).map(item => item._id).filter(Boolean))
...
function columns(data) {
  ...
  let tasks = []
  tasks =
    Object.keys(taskLikeStore.likeMap).length === 0 && data && data.length > 0
      ? (data || [])
          .map(item => ({ ...item, is_liked: true, like_count: item.like_count }))
          .filter(Boolean)
      : Object.values(taskLikeStore.likeMap)
...
function onLike(item) {
  ...
  // 乐观UI：先本地更新
  if (newLiked) {
    taskLikeStore.setLike(item._id, true, newCount, {
      ...item,
      is_liked: true,
      like_count: newCount
    })
  ...
  toggleTaskLike(item._id, oldLiked)
    .then(({ isLiked, likeCount }) => {
      if (isLiked) {
        taskLikeStore.setLike(item._id, true, likeCount, {
          ...item,
          is_liked: true,
    ...
    .catch(error_ => {
      if (oldLiked) {
        taskLikeStore.setLike(item._id, true, oldCount, {
          ...item,
          is_liked: true,
          like_count: oldCount
        })
