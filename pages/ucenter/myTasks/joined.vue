          ...
          <view class="progress-content" v-if="progressData">
            <view class="task-info">
              ...
              <text class="task-status">{{
                progressData.task.status === 'in_progress' ? '进行中' : '已完成'
              }}</text>
            </view>
              ...
              <view v-if="progressData.progress && progressData.progress.length > 0">
                <view v-for="member in progressData.progress" :key="member._id" class="member-item">
                  ...
                  <view v-if="progressData.task.status === 'not_started'" class="member-actions">
                    ...
                    <template
                      v-if="
                        progressData.isPublisher &&
                        progressData.isAlsoMember &&
                        !member.is_publisher
                      "
                    >
            ...
            <view class="action-buttons">
              ...
              <template v-if="progressData?.task?.status === 'in_progress'">
                ...
                >
                  ...
                  <text>{{
                    progressData.isPublisher && progressData.isAlsoMember ? '确认完成' : '结束任务'
                  }}</text>
                </button>
              </template>
            </view>
...
export default {
  ...
  computed: {
    ...
    isPublisherAndMember() {
      ...
      return this.progressData.progress.some(
        member => member._id === this.userId && member.is_publisher
      )
    },
    categoryItemStyle() {
  ...
  methods: {
    ...
    getSwipeOptions(task) {
      ...
      if (
        (status === 'not_started' || status === 'invalid' || myStatus === 'evaluated') &&
        isAlsoMember
      ) {
        // 发布者叫"删除"，参与者叫"退出"
    ...
    async fetchMyJoinedTasks({ reset = false } = {}) {
      ...
      if (
        ...
        now - this.joinedPageCache[cacheKey].ts < this.cacheExpire
      ) {
    ...
    onFilterTab(index) {
      ...
      if (
        ...
        // 检查缓存中的任务状态是否与当前分类匹配
        const currentFilter = this.filterOptions[index]
    ...
    async submitRate() {
      ...
      try {
        ...
        if (res.result && res.result.code === 0) {
          ...
          // 1. 直接更新当前卡片状态
          const index = this.tasks.findIndex(t => t._id === this.rateEditTaskId)
          if (index !== -1) {
            this.tasks[index].status = 'evaluated'
            this.tasks[index].rateInfo = {
              rate: this.rateEditValue,
              rate_comment: this.rateEditComment,
              rate_time: Date.now()
            }
    ...
    async onReadyOrPreJoin(task, status) {
      ...
      if (res.result && res.result.code === 0) {
        ...
        for (const cacheKey of Object.keys(this.joinedPageCache)) {
    ...
    async endTaskFromProgress() {
      ...
      if (res.result && res.result.code === 0) {
        ...
        this.handleTaskStatusChange(
          this.progressData.task._id,
          this.progressData.task.status,
          'finished'
        )
    ...
    async viewTaskReadinessStatus(task) {
      try {
        ...
        if (res.result && res.result.code === 0) {
          ...
          const allReady = nonPublisherMembers.every(m => m.status === 'ready')
