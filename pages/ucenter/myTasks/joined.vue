	...
	<view>
		<view class="filter-bar-scroll">
			...
			<scroll-view class="category-scroll" scroll-x scroll-with-animation :scroll-left="categoryScrollLeft"
				...
				<view v-for="(item, idx) in filterOptions" :key="idx" :id="'cat-' + idx" class="seg-item"
					...
					<uni-badge v-if="categoryCounts[item] > 0" :text="categoryCounts[item]" absolute="rightTop"
						size="small" type="error" :offset="[-3, -3]">
		...
		<view class="page-content">
			<uni-swipe-action>
				<uni-swipe-action-item v-for="task in tasks" :key="task._id" :right-options="getSwipeOptions(task)"
					...
					<uni-card :thumbnail="task.userInfo?.avatar_file?.url || defaultAvatar" :title="task.name"
						...
						:sub-title="formatTime(task.start_time) + ' ~ ' + formatTime(task.end_time)" :is-shadow="true"
						:is-full="false" margin="4px 6px">
						<view class="card-header-row">
							<view class="card-title-row">
								...
								<text
									class="reward-value">{{ task.mode === 'score' ? task.score + '积分' : task.price + '元' }}</text>
							</view>
							<view class="card-sub-row">
								<text class="meta-label">参与人数：</text>
								<text
									class="meta-value">{{ task.members?.length || 0 }}/{{ task.max_participants }}</text>
			...
			<uni-drawer ref="progressDrawer" mode="right" :mask-click="false" :width="rateDrawerWidth"
				:style="{zIndex: 1200}">
				<view
					style="padding:24px 20px;min-width:240px;max-width:90vw;display:flex;flex-direction:column;min-height:60vh;">
					<view class="progress-header">
						<text
							class="progress-title">{{ progressData.task.status === 'not_started' ? '就绪状态' : '任务进度' }}</text>
					...
					<view class="progress-content" v-if="progressData">
						<view class="task-info">
							...
							<text
								class="task-status">{{ progressData.task.status === 'in_progress' ? '进行中' : '已完成' }}</text>
						</view>
							...
							<view v-if="progressData.progress && progressData.progress.length > 0">
								<view v-for="member in progressData.progress" :key="member._id" class="member-item">
									<view class="member-avatar">
										<cloud-image v-if="member.avatar" :src="member.avatar" width="40rpx"
											height="40rpx" style="border-radius: 50%;" />
									...
									<view v-if="progressData.task.status === 'not_started'" class="member-actions">
										<!-- 发布者兼参与者：显示提醒和移除按钮（仅对非发布者参与者） -->
										<template
											v-if="progressData.isPublisher && progressData.isAlsoMember && !member.is_publisher">
										...
										<template
											v-else-if="!progressData.isPublisher && progressData.isAlsoMember && member._id === userId">
											<view v-if="member.status === 'preJoin'" class="action-btn"
												@click="setReady">
												<uni-icons type="checkmarkempty" size="16" color="#52c41a" />
											</view>
											<view v-else-if="member.status === 'ready'" class="action-btn"
												@click="cancelReady">
									...
									<view class="member-status">
										<template v-if="progressData.task.status === 'not_started'">
											...
											<text
												class="status-text">{{ member.status === 'ready' ? '已就绪' : '未就绪' }}</text>
										</template>
										...
										<template v-else-if="progressData.task.status === 'in_progress'">
											...
											<view v-else class="status-default">
												<text
													class="status-text">{{ member.status === 'preJoin' ? '待就绪' : member.status === 'ready' ? '已就绪' : '未知' }}</text>
											</view>
										</template>
									</view>
								</view>
					...
					<view class="progress-footer">
						<view class="action-buttons">
							...
							<template v-if="progressData?.task?.status === 'in_progress'">
								<button class="end-task-btn" @click="endTaskFromProgress"
		...
		<uni-drawer ref="rateDrawer" mode="right" :mask-click="false" :width="rateDrawerWidth" :style="{zIndex: 1200}">
			<view
				style="padding:24px 20px;min-width:240px;max-width:90vw;display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:60vh;">
		...
		<uni-drawer ref="rateEditDrawer" mode="right" :mask-click="false" :width="rateDrawerWidth"
			...
			<view
				...
				<button style="margin-top:8px;width:100%;"
					@click="() => { showRateEditDrawer=false; $refs.rateEditDrawer.close(); }">取消</button>
			</view>
		</uni-drawer>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			async fetchMyJoinedTasks({
				...
				if (!reset && this.page === 1 && this.joinedPageCache[cacheKey] && (now - this.joinedPageCache[
						cacheKey].ts < this.cacheExpire)) {
						...
						if (res.confirm) {
							...
							try {
								...
								if (delRes.result && delRes.result.code === 0) {
									uni.showToast({
										title: confirmText + '成功',
										icon: 'success'
									});
									// 更新任务计数
									if (delRes.result.publishedCount !== undefined) {
										mutations.setUserInfo({
											publishedCount: delRes.result.publishedCount
										});
									}
									if (delRes.result.joinedCount !== undefined) {
										mutations.setUserInfo({
											joinedCount: delRes.result.joinedCount
										});
									}
									...
									uni.showToast({
										title: delRes.result?.message || confirmText + '失败',
										icon: 'none'
									});
								}
							} catch (e) {
								uni.showToast({
									title: confirmText + '失败',
									icon: 'none'
								});
							} finally {
								uni.hideLoading();
			...
			async onReadyOrPreJoin(task, status) {
				...
				if (res.result && res.result.code === 0) {
					...
					const updateStatus = t => {
						if (t._id === task._id) t.myJoinStatus = status;
					};
					...
					// 任务已开始，设为已失效
					const updateStatus = t => {
						if (t._id === task._id) t.status = 'invalid';
					};
					this.tasks.forEach(updateStatus);
					Object.keys(this.joinedPageCache).forEach(cacheKey => {
						const cacheList = this.joinedPageCache[cacheKey]?.data?.tasks;
			...
			async onStartTask(task) {
				...
				if (res.result && res.result.code === 0) {
					uni.showToast({
						title: res.result.message || '任务已开始',
						icon: 'success'
					});
			...
			async endTask(task) {
				...
				if (res.result && res.result.code === 0) {
					uni.showToast({
						title: res.result.message || '任务已结束',
						icon: 'success'
					});
				...
				try {
					...
					if (res.result && res.result.code === 0 && res.result.categoryCounts) {
						...
						if (this.joinedPageCache[cacheKey]) {
							this.joinedPageCache[cacheKey].data.categoryCounts = {
								...res.result.categoryCounts
							};
						}
					}
			...
			async sendReminder(member) {
				try {
					...
					if (res.result && res.result.code === 0) {
						...
						uni.showToast({
							title: res.result?.message || '提醒发送失败',
							icon: 'none'
						});
					}
				} catch (error) {
			...
			async removeJoiner(member) {
				uni.showModal({
					title: '移除参与者',
					content: `确定要移除参与者"${member.nickname}"吗？`,
					confirmText: '确定移除',
					confirmColor: '#ff4757',
					success: async (res) => {
						if (res.confirm) {
