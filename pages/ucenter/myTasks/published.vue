			...
			<view
				...
				<view class="progress-content" v-if="progressData">
					<view class="task-info">
						...
						<text
							class="task-status">{{ progressData.task.status === 'not_started' ? '待开始' : progressData.task.status === 'in_progress' ? '进行中' : '已完成' }}</text>
					</view>
				...
				<view class="progress-footer">
					<view class="action-buttons">
						<button class="refresh-btn" @click="refreshProgress" :disabled="!progressData?.task?._id">
							...
							<text>{{ progressData?.task?.status === 'not_started' ? '刷新状态' : '刷新进度' }}</text>
						</button>

						<!-- 待开始状态：显示开始任务按钮 -->
						<template v-if="progressData?.task?.status === 'not_started'">
							<button class="end-task-btn" @click="startTaskFromProgress"
								:disabled="!progressData?.allReady" :class="{ disabled: !progressData?.allReady }">
	...
	export default {
		...
		methods: {
			...
			async fetchMyPublishedTasks({
				...
				if (!reset && this.page === 1 && this.publishedPageCache[cacheKey] && (now - this.publishedPageCache[
			...
			onFilterExtra(idx) {
				if (this.filterExtraIndex !== idx) {
					...
					const cacheKey = this.getCacheKey();
					const now = Date.now();
					if (this.publishedPageCache[cacheKey] && (now - this.publishedPageCache[cacheKey].ts < this
							.cacheExpire)) {
						const cached = this.publishedPageCache[cacheKey].data;
				...
				try {
					...
					if (res.result && res.result.code === 0) {
						...
						if (task.status === 'not_started') {
							// 待开始状态：计算是否所有非发布者参与者都已就绪
							const nonPublisherMembers = progress.filter(m => !m.is_publisher);
							allReady = nonPublisherMembers.length === 0 ||
								nonPublisherMembers.every(m => m.status === 'ready');
						} else if (task.status === 'in_progress') {
							// 进行中状态：计算是否所有参与者都已完成
							allFinished = progress.length === 0 ||
								progress.every(m => m.status === 'finished');
						}
			...
			async refreshProgress() {
				...
				try {
					...
					if (res.result && res.result.code === 0) {
						...
						if (this.progressData.task.status === 'not_started') {
							// 待开始状态：计算是否所有非发布者参与者都已就绪
							const nonPublisherMembers = progress.filter(m => !m.is_publisher);
							allReady = nonPublisherMembers.length === 0 ||
								nonPublisherMembers.every(m => m.status === 'ready');
						} else if (this.progressData.task.status === 'in_progress') {
							// 进行中状态：计算是否所有参与者都已完成
							allFinished = progress.length === 0 ||
								progress.every(m => m.status === 'finished');
						}
			...
			async startTaskFromProgress() {
				...
				if (res.result && res.result.code === 0) {
					...
					// 使用简化的状态变更处理方法
					this.handleTaskStatusChange(this.progressData.task._id, this.progressData.task.status,
						'in_progress');
			...
			async refreshCurrentCategory() {
				try {
					...
					// 强制重新获取数据，不使用缓存
					await this.fetchMyPublishedTasks({
						reset: true
					});
