        ...
        <view class="progress-content" v-if="progressData">
          <view class="task-info">
            ...
            <text class="task-status">{{
              progressData.task.status === 'not_started'
                ? '待开始'
                : progressData.task.status === 'in_progress'
                  ? '进行中'
                  : '已完成'
            }}</text>
          </view>
  ...
  methods: {
    ...
    async fetchMyPublishedTasks({ reset = false } = {}) {
      ...
      if (
        ...
        now - this.publishedPageCache[cacheKey].ts < this.cacheExpire
      ) {
      ...
      try {
        ...
        if (res.result && res.result.code === 0) {
          ...
          if (task.status === 'not_started') {
            ...
            allReady = nonPublisherMembers.every(m => m.status === 'ready')
          } else if (task.status === 'in_progress') {
            // 进行中状态：计算是否所有参与者都已完成
            allFinished = progress.every(m => m.status === 'finished')
          }
