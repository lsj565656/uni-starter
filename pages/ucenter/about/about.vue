...
export default {
  ...
  computed: {
    ...
    agreements() {
      ...
      const { serviceUrl, privacyUrl } = uniIdPagesConfig.agreements
      return [
        {
          url: serviceUrl,
          title: '用户服务协议'
  ...
  onNavigationBarButtonTap() {
    const { download, appName, slogan, logo } = this.about
    uniShare.show(
      {
        content: {
  ...
  methods: {
    navigateTo({ url, title }) {
      uni.navigateTo({
        url:
          '/uni_modules/uni-id-pages/pages/common/webview/webview?url=' + url + '&title=' + title,
        success: res => {},
        fail: () => {},
        complete: () => {}
      })
